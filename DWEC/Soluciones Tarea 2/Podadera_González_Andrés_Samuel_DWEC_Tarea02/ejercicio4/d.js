...
if (fechaUsuario == "Invalid Date") {
  ...
  if (
    fechaUsuario.toLocaleDateString("es-ES") ===
    ...
    respuesta += dias > 1 ? `s` : "";
    respuesta += esMayorFechaActual
      ? ` desde la fecha introducida ${fechaUsuario.toLocaleDateString(
          "es-ES"
        )}`
      : ` para que llegue la fecha introducida ${fechaUsuario.toLocaleDateString(
          "es-ES"
        )}`;
  }
