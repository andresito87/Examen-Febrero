...
if (
    !defined('MEDIOS_CONTACTO') || !is_array(MEDIOS_CONTACTO) || !isset($detalle_usuario) || !is_array($detalle_usuario) ||
    !isset($detalle_usuario['id'])
) {
        ...
        <label for="empleado">Empleado:
            <select id="idempleado" name="empleado">
                <?php foreach ($empleados as $datosEmpleado) : ?>
                    <option value="<?= $datosEmpleado['id'] ?>" <?php if (isset($datos) && $datos['empleado_id'] === $datos['empleado_id']) echo "selected"; ?>>
                        <?= htmlspecialchars($datosEmpleado['nombre'] . ' ' . $datosEmpleado['apellidos']) ?></option>
                <?php endforeach; ?>
